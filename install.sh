#!/bin/bash
# =============================================================================
# VPN Infrastructure Installer v3.0 (Optimized)
# Без лишнего Nginx, с гибкой настройкой портов
# =============================================================================
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║     VPN Infrastructure Installer v3.0                  ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════╝${NC}"

# 1. Система + Docker
echo -e "${YELLOW}[1/5] Система + Docker...${NC}"
apt update && apt upgrade -y
apt install -y curl wget git socat ufw
...
#!/bin/bash
BACKUP_DIR="/root/backups/hiddify"; DATE=$(date +%F_%H-%M); mkdir -p $BACKUP_DIR
...
#!/bin/bash
BACKUP_DIR="/root/backups/amnezia"; DATE=$(date +%F_%H-%M); mkdir -p $BACKUP_DIR
